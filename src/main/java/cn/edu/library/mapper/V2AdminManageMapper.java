...
public interface V2AdminManageMapper {
    ...
    @Select({
            "<script>",
            "SELECT id AS id, username AS username, real_name AS realName, role AS role,",
            "       phone AS phone, email AS email, status AS status, create_time AS createTime, update_time AS updateTime",
            "FROM admin",
            "WHERE role = 'ADMIN'",
    ...
    @Select({
            "<script>",
            "SELECT id AS id, operator_type AS operatorType, operator_id AS operatorId, operator_name AS operatorName,",
            "       module AS module, operation AS operation, request_url AS requestUrl, ip AS ip,",
            "       result AS result, create_time AS createTime",
            "FROM operation_log",
            "WHERE 1 = 1",
